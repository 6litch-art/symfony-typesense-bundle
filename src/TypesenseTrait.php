    ...
    public function __typesenseGetter(string $propertyName, array $propertyInfo): mixed
    {
        ...
        if ($value instanceof Collection) {
            ...
            $value = $value->Map(function($v) use ($propertyName) {
                ...
                if($v instanceof TypesenseInterface) return $v->__typesense();
                if(is_object($v) && method_exists($v, "__toString")) return $v->__toString();
        ...
        if(is_array($v)) {
            ...
            $flattenArray = [];
            array_walk_recursive($v, function($a) use (&$flattenArray) { $flattenArray[] = $a; });
